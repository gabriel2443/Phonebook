...
internal class UserInput
{
    ...
    public void Menu()
    {
        ...
        while (isRunning)
        {
            ...
            select.AddChoice("Exit the applitcation");
    ...
    internal void AddContactDetail()
    {
    ...
    internal void UpdateDetails()
    {
        ...
        string name = AnsiConsole.Ask<string>($"Please enter the [green]name[/] you want to update?");

        string email = AnsiConsole.Ask<string>($"Please enter the [green]email[/] you want to update?");
        while (!Helpers.IsValidEmail(email))
        {
            AnsiConsole.Markup("[red]Invalid email format, please enter another email[/]");
            email = AnsiConsole.Ask<string>("Please enter the [green]email[/] you want to update?");
        }

        string phoneNum = AnsiConsole.Ask<string>($"Please enter the [green]phone number[/] in the [bold]U.S[/] format?");
        while (!Helpers.IsValidPhoneNum(phoneNum))
        {
            AnsiConsole.Markup("[red]Invalid phone number format, please enter another phone number[/]");
            phoneNum = AnsiConsole.Ask<string>("Please enter the [green]phone number[/] in the [bold]U.S[/] format?");
        }

        contactService.EditContacts(name, email, phoneNum);
    }
